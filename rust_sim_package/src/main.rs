...
impl Environment {
    ...
    fn timeout(&mut self, agent:&Agent, process_time:u64) {
    ...
    fn enter_queue(&mut self, agent:&Agent, process_time:u64, target:String) {
    ...
    fn resource_interruption(&mut self, interruption:ResourceInterruption) {

        if let Some(resource) = self.resources.get_mut(&interruption.target) {
            resource.interruption_queue.push((Reverse(interruption.spawn_time), interruption));
        } else {
            println!(
                "Failed adding interruption id {}. Resource target {} does not exist.",
                interruption.id, interruption.target
            );
        }
                ...
                if event.scheduled_time == sim_time {
                    ...
                    match event.event_type {
                        ...
                        EventType::Timeout => {
                            ...
                            sim_logs.push(
                                format!("{}: Agent id {} is executing event {}.", sim_time, event.id, event.event_type)
                            );
                        ...
                        EventType::EnterQueue => {
                            ...
                            sim_logs.push(
                                format!("{}: Agent id {} is being added to {}.", sim_time, event.id, event.target)
                            );
                    ...
                    .filter(|(_, resource_capacity)| resource_capacity.available == sim_time)
                {
                    ...
                    if let Some(event_queue) = self.event_list.get_mut(&resource_capacity.current_agent_id) {
                        if let Some(idle_event) = event_queue.pop_front() {
                            ...
                            sim_logs.push(
                                format!(
                                    "{}: Agent id {} has event type {} moved from events list to scheduled events.",
...
struct Resource {
    ...
    interruption_queue:BinaryHeap<(Reverse<u64>, ResourceInterruption)>,
}
...
impl Resource {
    ...
    fn new_resource(capacity:u64, resource_name:String, queue_target:String) -> Resource {
        ...
        let mut new_resource:Resource = Resource {
            ...
            status:HashMap::new(),
            interruption_queue:BinaryHeap::new(),
        };
...
fn main() {
    ...
    let agents:Vec<Agent> = vec![
        Agent {id:1,spawn_time:3,process_time:1},
        Agent {id:2,spawn_time:3,process_time:7},
        Agent {id:3,spawn_time:3,process_time:10},
    ];
    ...
    let interruptions:Vec<ResourceInterruption> = vec![
        ResourceInterruption {
            id:1,
            spawn_time:1,
            target:"resource_1".to_string(),
            capacity_interrupted:2,
            duration:2,
        }
    ];
    
    for agent in agents.into_iter() {
        agent.process(&mut sim_env);
    }
    ...
    for interruption in interruptions.into_iter() {
        sim_env.resource_interruption(interruption);
    }
